...
func (app *application) signupUser(w http.ResponseWriter, r *http.Request) {
	...
	if !f.Valid() {
		app.render(w, r, "signup.page.tmpl", &templateData{Form: f})
		return
	}

	err = app.users.Insert(f.Get("name"), f.Get("email"), f.Get("password"))
	if err == models.ErrDuplicateEmail {
		f.Errors.Add("email", "Address is already in use")
		app.render(w, r, "signup.page.tmpl", &templateData{Form: f})
		return
	} else if err != nil {
		app.serverError(w, err)
	...
	app.session.Put(r, "flash", "Hey, welcome "+f.Get("name")+"! Please login.")
